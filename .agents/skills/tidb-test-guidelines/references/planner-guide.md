...
- For hint tests, preserve the existing format when the test asserts warning text, hint applicability, or other format-sensitive output. Only move a hint test to `plan_tree` when the nearby suite already uses `plan_tree` for that behavior.
