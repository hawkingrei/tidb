...
func TestJoinReorderWithAddSelection(t *testing.T) {
	...
	tk.MustQuery(`explain select 0 from t2 join(t3 join t0 a on 0) left join(t1 b left join t1 c on 0) on(c20 = b.vkey) on(c13 = a.vkey) join(select c14 d from(t2 join t3 on c12 = vkey)) e on(c3 = d) where nullif(c15, case when(c.c10) then 0 end);`).Check(testkit.Rows(
		...
		`└─HashJoin_48 0.00 root  inner join, equal:[eq(Column#27, Column#28)]`,
		...
		`  │ │ └─HashJoin_72 0.00 root  left outer join, left side:HashJoin_86, equal:[eq(test.t3.c20, test.t1.vkey)]`,
		`  │ │   ├─HashJoin_86(Build) 0.00 root  inner join, equal:[eq(test.t0.vkey, test.t2.c13)]`,
		`  │ │   │ ├─TableDual_95(Build) 0.00 root  rows:0`,
		`  │ │   │ └─TableReader_94(Probe) 9990.00 root  data:Selection_93`,
		`  │ │   │   └─Selection_93 9990.00 cop[tikv]  not(isnull(test.t2.c13))`,
		`  │ │   │     └─TableFullScan_92 10000.00 cop[tikv] table:t2 keep order:false, stats:pseudo`,
		`  │ │   └─HashJoin_102(Probe) 9990.00 root  CARTESIAN left outer join, left side:TableReader_106`,
		`  │ │     ├─TableDual_107(Build) 0.00 root  rows:0`,
		`  │ │     └─TableReader_106(Probe) 9990.00 root  data:Selection_105`,
		`  │ │       └─Selection_105 9990.00 cop[tikv]  not(isnull(test.t1.vkey))`,
		`  │ │         └─TableFullScan_104 10000.00 cop[tikv] table:b keep order:false, stats:pseudo`,
		`  │ └─Projection_113(Probe) 9990.00 root  test.t2.c14, cast(test.t2.c12, double BINARY)->Column#27`,
		`  │   └─TableReader_116 9990.00 root  data:Selection_115`,
		`  │     └─Selection_115 9990.00 cop[tikv]  not(isnull(test.t2.c14))`,
		`  │       └─TableFullScan_114 10000.00 cop[tikv] table:t2 keep order:false, stats:pseudo`,
		`  └─Projection_119(Probe) 10000.00 root  cast(test.t3.vkey, double BINARY)->Column#28`,
		`    └─TableReader_121 10000.00 root  data:TableFullScan_120`,
		`      └─TableFullScan_120 10000.00 cop[tikv] table:t3 keep order:false, stats:pseudo`,
	))
}
