...
func TestABC(t *testing.T) {
  ...
  game_category_id tinyint NOT NULL DEFAULT '0' COMMENT '游戏类型',
  game_platform_id int NOT NULL DEFAULT '0' COMMENT '游戏平台',
  game_id int NOT NULL DEFAULT '0' COMMENT '游戏id',
  betitem_num int NOT NULL DEFAULT '0' COMMENT '单量',
  bet_total decimal(20,6) NOT NULL DEFAULT '0' COMMENT '投注金额',
  valid_bet_total decimal(20,6) NOT NULL DEFAULT '0' COMMENT '有效投注金额',
  profit_total decimal(20,6) NOT NULL DEFAULT '0' COMMENT '会员盈利',
  max_valid_bet decimal(20,6) NOT NULL DEFAULT '0' COMMENT '最大有效投注',
  max_profit decimal(20,6) NOT NULL DEFAULT '0' COMMENT '会员最大盈利',
  min_profit decimal(20,6) NOT NULL DEFAULT '0' COMMENT '会员最大亏损',
  settle_day int NOT NULL DEFAULT '0' COMMENT '注单结算YMD',
  currency varchar(50) NOT NULL DEFAULT '' COMMENT '币种',
  bet_create_time varchar(50) NOT NULL DEFAULT '' COMMENT '注单创建时间',
  bet_update_time varchar(50) NOT NULL DEFAULT '' COMMENT '注单更新时间',
  create_time int NOT NULL DEFAULT '0' COMMENT '创建时间',
  update_time int NOT NULL DEFAULT '0' COMMENT '更新时间',
  PRIMARY KEY (id) /*T![clustered_index] NONCLUSTERED */,
  KEY idx_active_bet_summary_update_time (update_time),
  KEY idx_active_bet_summary_game_category_id (game_category_id,settle_day),
  KEY idx_active_bet_summary_useridx (useridx,settle_day),
  KEY idx_active_bet_summary_game_bet_update_time (bet_update_time),
  UNIQUE KEY udx_dh_active_bet_summary_sitecode (useridx,game_category_id,game_platform_id,game_id,settle_day,site_code),
  KEY idx_active_bet_summary_game_useridx_sitecode (useridx,site_code,settle_day),
  KEY idx_dh_active_bet_summary_settleday_betupdatetime_sitecode (settle_day,bet_update_time,site_code),
  KEY idx_day_currency_category_platform_sitecode (settle_day,currency,game_category_id,game_platform_id,site_code),
  KEY idx_active_bet_summary_updatetime_useridx_sitecode (update_time,useridx,site_code),
  ...
  site_code,
  useridx,
  SUM(betitem_num) betitem_num,
  SUM(valid_bet_total) valid_bet_total,
  SUM(profit_total) profit_total,
  MAX(max_valid_bet) max_valid_bet,
  MAX(max_profit) max_profit,
  MIN(min_profit) min_profit,
  MAX(settle_day) settle_day,
  MAX(bet_update_time) bet_update_time
FROM
  dh_active_bet_summary
WHERE
  site_code = "552"
  AND settle_day >= 20251103
  AND settle_day <= 20251109
  AND game_id >= 0
  AND useridx IN (
    SELECT
      DISTINCT useridx
    FROM
      dh_active_bet_summary
    WHERE
      bet_update_time >= "2025-11-08 07:50:21"
      AND site_code = "552"
