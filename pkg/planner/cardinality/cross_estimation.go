...
func convertRangeFromExpectedCnt(ranges []*ranger.Range, rangeCounts []float64, expectedCnt float64, desc bool) ([]*ranger.Range, float64, bool) {
	...
	if desc {
		...
		convertedRanges = []*ranger.Range{{LowVal: ranges[i].HighVal, HighVal: []*types.Datum{&ranger.MaxValueDatum}, LowExclude: !ranges[i].HighExclude, Collators: ranges[i].Collators}}
	} else {
		for i = range ranges {
			if count+rangeCounts[i] >= expectedCnt {
				break
