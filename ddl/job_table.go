...
func generateInsertBackfillJobSQL(name string, backfillJobs []*BackfillJob) (string, error) {
	...
	for i, bj := range backfillJobs {
		...
		sqlBuilder.WriteString(fmt.Sprintf("('%d_%s_%d_%d', %d, %d, '%s', '%s', '%s', %s, %d, %d, %s)",
			bj.JobID, hex.EncodeToString(bj.EleKey), bj.EleID, bj.ID, bj.StoreID, bj.Tp, bj.InstanceID, bj.InstanceLease, bj.State.String(), wrapKey2String(bj.Meta.CurrKey),
...
func GetAndMarkBackfillJobsForOneEle(s *session, batch int, jobID int64, uuid string, lease time.Duration) ([]*BackfillJob, error) {
	...
	err := s.runInTxn(func(se *session) error {
		...
		bJobs, err = GetBackfillJobs(se, BackgroundSubtaskTable,
			fmt.Sprintf("(exec_id = '' or exec_expired < '%v') and task_key like \"%d_%%\" order by task_key limit %d",
		...
		for i := 0; i < len(bJobs); i++ {
			...
			// TODO: batch update
			if err = updateBackfillJob(se, bJobs[i], "get_mark_backfill_job"); err != nil {
				return err
			}
		}
		return nil
...
func GetBackfillJobCount(sess *session, table, condition string, label string) (int, error) {
	sql := fmt.Sprintf("select count(1) from mysql.%s where %s", table, condition)
	rows, err := sess.execute(context.Background(), sql, label)
	if err != nil {
...
func GetBackfillJobs(sess *session, table, condition string, label string) ([]*BackfillJob, error) {
	sql := fmt.Sprintf("select * from mysql.%s where %s", table, condition)
	rows, err := sess.execute(context.Background(), sql, label)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
// GetBackfillHistoryJobs gets the backfill jobs in the tblName table according to condition.
func GetBackfillHistoryJobs(sess *session, condition string, label string) ([]*BackfillJob, error) {
	rows, err := sess.execute(context.Background(), fmt.Sprintf("select * from mysql.%s where %s", BackgroundSubtaskHistoryTable, condition), label)
	if err != nil {
		return nil, errors.Trace(err)
	}
	bJobs := make([]*BackfillJob, 0, len(rows))
	for _, row := range rows {
		bfJob := BackfillJob{
			...
			StateUpdateTS: row.GetUint64(13),
		}
		bfJob.Meta = &model.BackfillMeta{}
		...
		bfJob.Meta.CurrKey = row.GetBytes(9)
		bfJob.Meta.StartKey = row.GetBytes(10)
		bfJob.Meta.EndKey = row.GetBytes(11)
		bfJob.Meta.RowCount = row.GetInt64(14)
		bJobs = append(bJobs, &bfJob)
	}
	return bJobs, nil
}
...
func updateBackfillJob(sess *session, backfillJob *BackfillJob, label string) error {
	...
	sql := fmt.Sprintf("update mysql.%s set exec_id = '%s', exec_expired = '%s', state = '%s', checkpoint = %s, meta = %s where task_key = '%d_%s_%d_%d'",
		BackgroundSubtaskTable, backfillJob.InstanceID, backfillJob.InstanceLease, backfillJob.State.String(), wrapKey2String(backfillJob.Meta.CurrKey),
